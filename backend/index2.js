...
exports.stakeXVS = exports.stakeCake = exports.stakeLista = exports.addLiquidity = exports.checkBalance = exports.swapTokens = exports.wrapBNB = exports.transferTokens = void 0;
...
async function wrapBNB(amount) {
    try {
        ...
        if (bnbBalance < amountWei) {
            throw new Error(`Insufficient BNB balance. Have ${ethers_1.ethers.formatEther(bnbBalance)} BNB, need ${amount} BNB`);
        }
        // Initialize WBNB contract
        const wbnb = new ethers_1.ethers.Contract(CONTRACTS.WBNB, WBNB_ABI, wallet);
        ...
        console.log(`New WBNB balance: ${ethers_1.ethers.formatEther(wbnbBalance)} WBNB`);
        return receipt;
    }
    catch (error) {
...
async function swapTokens(tokenInAddress, tokenOutAddress, amountIn, slippageTolerance = 0.5) {
    try {
        ...
        if (tokenInAddress === CONTRACTS.WBNB) {
            const wbnb = new ethers_1.ethers.Contract(CONTRACTS.WBNB, WBNB_ABI, wallet);
            const wbnbBalance = await wbnb.balanceOf(wallet.address);
            const amountWei = ethers_1.ethers.parseUnits(amountIn, 18);
            if (wbnbBalance < amountWei) {
                console.log("Insufficient WBNB, wrapping BNB first...");
                await wrapBNB(amountIn);
            }
        }
        const router = new ethers_1.ethers.Contract(CONTRACTS.PANCAKESWAP_ROUTER, ROUTER_ABI, wallet);
        const tokenIn = new ethers_1.ethers.Contract(tokenInAddress, ERC20_ABI, wallet);
        // Get token decimals
        // const decimals = await tokenIn.decimals();
        const amountInWei = ethers_1.ethers.parseUnits(amountIn, 18);
        // Check balance
        ...
        const approveTx = await tokenIn.approve(CONTRACTS.PANCAKESWAP_ROUTER, amountInWei);
        ...
        const swapTx = await router.swapExactTokensForTokens(amountInWei, amountOutMin, [tokenInAddress, tokenOutAddress], wallet.address, deadline);
...
async function checkBalance(tokenAddress) {
    ...
    const decimals = await token.decimals();
    const balance = await token.balanceOf(wallet.address);
    console.log(`Balance: ${ethers_1.ethers.formatUnits(balance, decimals)} tokens`);
}
exports.checkBalance = checkBalance;
async function addLiquidity(tokenAAddress, tokenBAddress, amountA, amountB, slippageTolerance = 0.5) {
    try {
        const provider = new ethers_1.ethers.JsonRpcProvider(RPC_URL);
        const wallet = new ethers_1.ethers.Wallet(process.env.PRIVATE_KEY, provider);
        // Initialize contracts
        const router = new ethers_1.ethers.Contract(CONTRACTS.PANCAKESWAP_ROUTER, ROUTER_ABI_EXTENDED, wallet);
        const tokenA = new ethers_1.ethers.Contract(tokenAAddress, ERC20_ABI, wallet);
        const tokenB = new ethers_1.ethers.Contract(tokenBAddress, ERC20_ABI, wallet);
        // Get decimals
        const decimalsA = await tokenA.decimals();
        const decimalsB = await tokenB.decimals();
        // Convert amounts to Wei
        const amountAWei = ethers_1.ethers.parseUnits(amountA, decimalsA);
        const amountBWei = ethers_1.ethers.parseUnits(amountB, decimalsB);
        // Calculate min amounts with slippage
        const amountAMin = (amountAWei * BigInt(Math.floor((100 - slippageTolerance) * 100))) /
            BigInt(10000);
        const amountBMin = (amountBWei * BigInt(Math.floor((100 - slippageTolerance) * 100))) /
            BigInt(10000);
        // Approve router for both tokens
        console.log("Approving tokens...");
        const approveA = await tokenA.approve(CONTRACTS.PANCAKESWAP_ROUTER, amountAWei);
        await approveA.wait();
        const approveB = await tokenB.approve(CONTRACTS.PANCAKESWAP_ROUTER, amountBWei);
        await approveB.wait();
        console.log("Approvals confirmed");
        // Add liquidity
        console.log("Adding liquidity...");
        const deadline = Math.floor(Date.now() / 1000) + 300; // 5 minutes
        const tx = await router.addLiquidity(tokenAAddress, tokenBAddress, amountAWei, amountBWei, amountAMin, amountBMin, wallet.address, deadline);
        const receipt = await tx.wait();
...
async function stakeLista(amount) {
    try {
        const provider = new ethers_1.ethers.JsonRpcProvider(RPC_URL);
        const wallet = new ethers_1.ethers.Wallet(process.env.PRIVATE_KEY, provider);
        // First approve SLISBNB token spending
        const slisToken = new ethers_1.ethers.Contract(CONTRACTS.SLISBNB, ERC20_ABI, wallet);
        const amountWei = ethers_1.ethers.parseUnits(amount, 18);
        const stakingContract = new ethers_1.ethers.Contract(CONTRACTS.SLISBNB, LISTA_STAKING_ABI, wallet);
...
exports.stakeLista = stakeLista;
async function stakeCake(amount) {
    try {
        const provider = new ethers_1.ethers.JsonRpcProvider(RPC_URL);
        const wallet = new ethers_1.ethers.Wallet(process.env.PRIVATE_KEY, provider);
        // First swap WBNB to CAKE if needed
        const cake = new ethers_1.ethers.Contract(CONTRACTS.CAKE, CAKE_TOKEN_ABI, wallet);
        ...
        const cakePool = new ethers_1.ethers.Contract(CONTRACTS.CAKE_POOL, CAKE_POOL_ABI, wallet);
        ...
        if (userLock.amount === BigInt(0) && userLock.end === BigInt(0)) {
            ...
            console.log(`Creating new CAKE lock with amount ${amountWei} until timestamp ${UNLOCK_TIME}...`);
...
exports.stakeCake = stakeCake;
async function stakeXVS(amount) {
    try {
        const provider = new ethers_1.ethers.JsonRpcProvider(RPC_URL);
        const wallet = new ethers_1.ethers.Wallet(process.env.PRIVATE_KEY, provider);
        // First swap WBNB to XVS if needed
        const xvs = new ethers_1.ethers.Contract(CONTRACTS.XVS, XVS_TOKEN_ABI, wallet);
        ...
        await xvs.approve(CONTRACTS.XVS_POOL, ethers_1.ethers.MaxUint256);
        // Deposit XVS
        const xvsPool = new ethers_1.ethers.Contract(CONTRACTS.XVS_POOL, vaultABI, wallet);
